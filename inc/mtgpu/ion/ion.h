...
#define ion_device_add_heap(dev, heap) \
	__ion_device_add_heap(dev, heap, THIS_MODULE)
...
int ion_device_destroy(struct ion_device *idev);
 ...
 */
int __ion_device_add_heap(struct ion_device *idev, struct ion_heap *heap,
			  struct module *owner);
 ...
 */
struct dma_buf *ion_alloc(struct ion_device *idev, size_t len,
...
size_t ion_query_heaps_kernel(struct ion_device *idev,
			      struct ion_heap_data *hdata, size_t size);
#endif /* _ION_KERNEL_H */
